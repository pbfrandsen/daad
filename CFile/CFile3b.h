...
// 1.10.2013: Bug discovered:
// Crashed after opening a lot of files. Only seen in exon_cutter program.
// Possible reason: istream objects had never been deleted. A few thousand open
//                  istream apparently cause a problem and are a memory leak.
//                  The close command will now delete the istream object.
//                  An open command will create a new istream object.
//
...
class CFile
{
 ...
 private:
  ...
  unsigned fill_buffer(unsigned overlap)
  {
    ...
    // Initially, buffer_end == buffer, so good_overlap == 0
    // This prevents problems when first using this function.
 ...
 public:
  ...
  void ffclose()
  {
    if (is_ifstream && this_is)
    {
      ((std::ifstream *)this_is)->close();
      delete this_is;
      this_is = NULL; // Set to NULL prevents it to be deleted twice.
    }
  }
