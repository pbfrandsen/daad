...
//   }
  ...
  cout << "Test non_ambig_non_gap coverage: " << endl;
  cout << "Range " << x1 << ", " << x2 <<" with flag: " << flag << endl;
  cout.setf(ios::fixed);
  cout.precision(12);
  cout << seqs.get_non_ambig_non_gap_coverage_in_range(x1, x2, flag);
  cout << endl;
  cout << endl;
...
int test_phylip(const char *fn)
{
  ...
  seqs.ExportSequences_no_fill_in_ext_phylip_range(cout, coords, 3);
...
int main(int argc, const char ** argv)
{
  if (argc != 3)
  {
    cerr << "Usage: a.out fas|phy filename" << endl;
    exit(0);
  }

  faststring typestr  = argv[1];
  faststring filename = argv[2];

  if (typestr == "fas")
  {
    test_fasta(filename.c_str(), 0);
    test_fasta(filename.c_str(), 1);
  }
  else   if (typestr == "phy")
    test_phylip(filename.c_str());
  else 
    cerr << "Unknown file format. Known file formats are: fas and phy" << endl; 
